...
void TCPConnection::segment_received(const TCPSegment& seg) {
  ...
  // 还没有建立连接，不能接收数据（payload().size() > 0）
  if (in_syn_sent() && seg.header().ack && seg.payload().size() > 0) {
    return;
  }
  bool send_empty = false;
  // 如果已经发送过数据（包括SYN），处理对方发送的ACK
  // 如果ACK合法，更新sender相关信息
  // 如果ACK不合法，发送一个空段纠正
  ...
  if (send_empty) {
    if (_receiver.ackno() && _sender.segments_out().empty()) {
...
void TCPConnection::connect() { push_segments_out(true); }
